...
DROP SEQUENCE  IF EXISTS JIRA_PROJECT_WORKER_id_seq CASCADE;
CREATE SEQUENCE JIRA_PROJECT_WORKER_id_seq START WITH 1000;
DROP TABLE  IF EXISTS JIRA_PROJECT_WORKER  CASCADE;
CREATE TABLE JIRA_PROJECT_WORKER (
  id INT NOT NULL DEFAULT NEXTVAL('JIRA_PROJECT_WORKER_id_seq') PRIMARY KEY,
  jira_Project_Id INTEGER,
   JIRA_WORKER_Id INTEGER ,
  FOREIGN KEY (jira_Project_Id) REFERENCES Jira_Project(id) ON DELETE CASCADE,
  FOREIGN KEY (JIRA_WORKER_Id) REFERENCES JIRA_WORKER(id)
);