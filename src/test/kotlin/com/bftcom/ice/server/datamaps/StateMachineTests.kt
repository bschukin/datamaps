...
import com.bftcom.ice.server.datamaps.StateMachineTests.GuessNumber.tryCount
...
open class StateMachineTests : BaseSpringTests() {
    ...
    fun testStateMachineCrud() {
        ...
        val states = dataService.findAll(State.filter {
            { f(State.stateMachine().id) eq value(sm2.id) } and {
                f(State.id) neq value(trans[0][Transition.state].id)
            }
        })
        println(states)
    ...
    private fun buildStateMachine(): Any? {
        val sm = StateMachine {
            ...
            two[{ transitions }].add(Transition.create {
                ...
                it[target] = three
                it[{ action }] = Action {
                    it[Action.className] = DogovorAction::class.java.name
                    it[Action.method] = DogovorAction::applyFinalAction.name
                }
            })
