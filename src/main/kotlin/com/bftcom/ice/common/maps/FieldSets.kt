...
import com.bftcom.ice.common.general.ValidationError
import com.bftcom.ice.common.general.ValidationException
...
abstract class FieldSet(private var _entity: String = "", var caption: String? = null,
    ...
    fun isDynamic() = containsOption(Dynamic::class) || entity == DataMap.DYNAMIC_ENTITY
...
open class MappingFieldSet<T : FieldSet>(entity: String = "",
    ...
    protected open fun applyDefaultValues(c: DataMap) {
        ...
        defaultValueFields
                .filter { it.fieldName.equals(DataMap.ID, true) }
                .union(
                        defaultValueFields.filter { !it.fieldName.equals(DataMap.ID, true) }
                )
                .forEach {
...
open class Field<T, L> private constructor(
    ...
    companion object {
        ...
        fun <T : FieldSet> reference(aname: String, t: T, handler: FieldHandler<DataMapF<T>?>? = null)
                : Field<T, DataMapF<T>?> {
            return Field<T, DataMapF<T>?>(aname, t, DataMap.empty() as DataMapF<T>, fieldType = FieldType.Long)
                    .let { field -> handler?.invoke(field); field }
        }

        fun <T : FieldSet> referenceNN(aname: String, t: T, handler: FieldHandler<DataMapF<T>>? = null): Field<T, DataMapF<T>> {
            return Field(aname, t, DataMap.empty() as DataMapF<T>, required = true, fieldType = FieldType.Long)
                    .let { field -> handler?.invoke(field); field }
        }

        fun reference(aname: String, nameOfReferencedEntity: String,
                      handler: FieldHandler<DataMap>? = null): Field<SomeFieldSet, DataMap> {
            return Field(aname, SomeFieldSet, DataMap.empty(), fieldType = FieldType.Long)
        ...
        = null): Field<T, MutableList<DataMapF<T>>> {
            return Field(aname, t, DataMap.emptyList() as MutableList<DataMapF<T>>,
        ...
        fun list(aname: String, nameOfReferencedEntity: String,
            ...
            return Field(aname, SomeFieldSet, DataMap.emptyList(), fieldType = FieldType.Other)
                    .apply { this.referencedEntity = nameOfReferencedEntity }
                    .let { field -> handler?.invoke(field); field }
        ...
        fun list(aname: String, handler: FieldHandler<MutableList<DataMap>>? = null): Field<SomeFieldSet, MutableList<DataMap>> {
            return Field(aname, SomeFieldSet, DataMap.emptyList(), fieldType = FieldType.Other).let { field -> handler?.invoke(field); field }
        }
        ...
        fun jsonbd(aname: String, handler: FieldHandler<DataMap?>? = null): Field<DataMap?, DataMap?> {
            return Field<DataMap?, DataMap?>(aname, DataMap.empty(), DataMap.empty(),
        ...
        fun <T : FieldSet> jsonObj(aname: String, t: T, handler: FieldHandler<DataMapF<T>?>? = null)
                : Field<T, DataMapF<T>?> {
            return Field<T, DataMapF<T>?>(aname, t, DataMap.empty() as DataMapF<T>, fieldType = FieldType.Json)
                    .let { field -> handler?.invoke(field); field }
        }
...
object DynamicEntity : MFS<DynamicEntity>(DataMap.DYNAMIC_ENTITY, "") {
    fun copy(dataMap: DataMap?): DataMapF<DynamicEntity> {
        return DataMapF(this, null, true,
                dataMap?.map?.apply {
                    this.remove(com.bftcom.ice.common.maps.DataMap.DYNAMIC_HOLDER)
                })
    }
}
