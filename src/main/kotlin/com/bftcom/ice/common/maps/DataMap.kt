...
import com.bftcom.ice.common.general.CaseInsensitiveKeyMap
import com.bftcom.ice.common.general.FieldType
import com.bftcom.ice.common.general.caseInsMapOf
...
open class DataMapF<T : FieldSet> {
    ...
    var id: Any? = null
        set(value) {
            ...
            field = convertMapToGuidOrPassBy(value)
        }
    ...
    constructor  (fieldSet: T?, name: String, id: Any? = null,
        ...
        if (isNew) {
            ...
            id?.let {
                DeltaStore.delta(this, DataMap.ID, null, id)
            }
        }
    }
    ...
    fun isDynamic(): Boolean = DataMap.DYNAMIC_ENTITY == entity || fieldSet?.isDynamic() == true
    ...
    operator fun get(field: String): Any? {
        if (DataMap.ID.equals(field, true))
            return id
        if (field.contains('.')) {
            return nested(field)
        }
    ...
    fun toReadonly(): DataMapF<T> {
        return DataMapF(fieldSet = this.fieldSet?:UndefinedFieldSet, id = id,
...
//гарантированно конвертирует ID после (возможной ) сериализации в "нормальный" вид
private fun convertMapToGuidOrPassBy(value: Any?): Any? {
...
fun FieldSet.findDisplayField(): Field<*, String>? {
    ...
    var field = getOption(DisplayName::class)?.fieldName?.let { displayFieldName -> fields.first { it.fieldName == displayFieldName } }
            ?: fields.find { it.n == DataMap.DISPLAY_NAME || it.n == DataMap.CAPTION }
            ?: fields.find { it.n == DataMap.NAME }
...
fun DataMap.getDisplayName(): String {
    return fieldSet?.findDisplayField()?.let { this[it] } ?: (firstOf(*DataMap.captionFields) as String?).orEmpty()
}