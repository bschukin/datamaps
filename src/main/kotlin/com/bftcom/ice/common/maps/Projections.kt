...
import com.bftcom.ice.common.general.*
...
abstract class BaseProjection {
    //выражение  where
    var filter: exp? = null
    var transportFilter: TypedExpression? = null
...
sealed class Expression {
    ...
    private fun bop(right: Any, op: Operation): exp = when {
        ...
        else -> BinaryOP(this, right as? exp ?: value(right), op)
    }
...
open class OqlExpression(oql: String, params: Map<String, Any?> = emptyMap()) : exp() {
    ...
    val params: Map<String, Expression> = params.mapValues { (_, value) -> value as? exp ?: value(value) }
