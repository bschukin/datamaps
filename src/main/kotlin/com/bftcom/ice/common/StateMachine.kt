...
//МОДЕЛЬ ОСНОВАНА НА РЕАЛЬНЫХ СОБЫТИЯХ:
...
object StatesGroup : FieldSetGroup("states", "Статусы и переходы", SettingsGroup)
...
object StateMachine : MappingFieldSet<StateMachine>("StateMachine", "Статусные модели", StatesGroup) {
    ...
    fun DataMapF<StateMachine>.findStartState(): DataMapF<State>? {
        return this[StateMachine.states].find { st -> true == st[State.isStart] }
    }

    fun DataMapF<StateMachine>.findState(code: String): DataMapF<State>? {
        return this[StateMachine.states].find { st -> code == st[State.code] }
    }
